...
Usage:
    docker compose exec vertebrate-app python -m pytest \
        /app/code/tests/test_auth_audit.py -v --noconftest
"""
import os
import json
import pytest
...
# Must be set before any app import to suppress Kafka init errors
...
def test_user(app, base_role):
    """
    Creates a fresh test user before each test, deletes it (+ audit rows) after.
    Uses a unique username unlikely to clash with real data.
    """
    ...
    with app.app_context():
        ...
        if existing:
            AuditLog.query.filter_by(record_type='USER',
                                     record_id=str(existing.id)).delete()
    ...
    with app.app_context():
        ...
        AuditLog.query.filter_by(record_type='USER', record_id=str(uid)).delete()
...
class TestRegistrationAudit:
    ...
    def test_new_user_creates_create_entry(self, app, client, base_role):
        ...
        try:
            ...
            entry = get_latest_entry(app, 'CREATE', 'USER', user.id)
            ...
            with app.app_context():
                u = User.query.filter_by(username=TMP).first()
                if u:
                    AuditLog.query.filter_by(record_type='USER',
                                             record_id=str(u.id)).delete()
                    db.session.delete(u)
                    db.session.commit()
...
class TestUpdateUserAudit:
    ...
    def test_password_change_is_redacted(self, app, client, test_user):
        ...
        assert entry.old_value == '[REDACTED]', \
            f"old_value should be [REDACTED], got: {entry.old_value}"
        assert entry.new_value == '[REDACTED]', \
            f"new_value should be [REDACTED], got: {entry.new_value}"
    ...
    def test_username_change_logs_old_and_new_values(self, app, client, test_user):
        ...
        # Restore username so the fixture teardown can find and delete the user
        with app.app_context():
    ...
    def test_role_change_logs_old_and_new_role_names(self, app, client, test_user):
        ...
        assert test_user['role_name'] in entry.old_value, \
            f"Expected old role '{test_user['role_name']}' in old_value: {entry.old_value}"
        assert 'admin' in entry.new_value, \
            f"Expected 'admin' in new_value: {entry.new_value}"
        assert entry.change_reason == 'Role updated by admin'
        assert entry.checksum is not None
        client.post('/logoutUser', content_type='application/json')


# ─── 5. Checksum Integrity (21 CFR Part 11) ──────────────────────────────────
...
class TestChecksumIntegrity:
    ...
    def test_every_audit_entry_for_test_user_has_a_checksum(self, app, client,
        ...
        # Trigger a few actions to populate the table
        login(client, test_user['username'], test_user['password'])
        client.post('/logoutUser', content_type='application/json')
